...
function tableSpec(name) {
  // speed isn't in the table (it has no control yet) but it has to travel
  // with every preview, or an animated effect previews as a solid one
  const speed = (cfg && cfg.states[name] && cfg.states[name].speed);
  const tr = document.querySelector(`#states tr[data-name="${name}"]`);
  if (tr) return Object.assign(specOf(tr), {s: speed === undefined ? 1 : speed});
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/config":
            ...
            # "s" matters: without the speed field an animated effect renders
            # solid (PROTOCOL.md §5.2), so dropping it here made Try show a
            # different key from the one Save produces
            fields = {k: body[k]
                      for k in ("slot", "color", "effect", "brightness",
                                "s", "m", "sk", "sa")
                      if k in body}
